...
const xmock = type => {
    ...
    //         break;
    //     case 'cc_mc':
    //         mock = Chance.cc({type: 'mc'});
    //         break;
    //     case 'cc_visa':
    //         mock = Chance.cc({type: 'visa'});
    //         break;
    //     case 'cc_amex':
    //         mock = Chance.cc({type: 'amex'});
    //         break;
    //     case 'money_amt':
    //         mock = Chance.natural({min: 1, max: 100}) +chance.pad(Chance.natural({min: 1, max: 99}), 2);
    //         break;
    //     default:
    //         mock = 'unknown mock';
    // }
...
const objectMock = ({ allOf, properties, additionalProperties }) => {
    let mockObj = {};

    /**
     * Solves #19 - mocks for properties defined by allof values are not getting generated.
     * extends props objects of allOf is present.
     */
    if (allOf) {
        ...
        allOf.forEach((ref) => {
            ...
            Object.keys(ref.properties).forEach(function (key) {
                properties[key] = ref.properties[key];
            });
        });
    }
...
const integerMock = schema => {
    ...
    if (schema['x-example']) {
        /**
         * If a `x-example` is defined for the property, use that.
         */
...
const numberMock = schema => {
    ...
    if (schema['x-example']) {
        /**
         * If a `x-example` is defined for the property
         */
        nummock = schema['x-example'];
        return nummock;
    }
...
const stringMock = schema => {
    ...
    if (schema.enum && schema.enum.length > 0) {
         ...
         * If a `x-example` is defined for the property
         */
        mockStr = xmock(schema['x-mock']);
    } else if (schema['x-example']) {
        /**
         * If a `x-example` is defined for the property
         */
        mockStr = schema['x-example'];
